...
def dashboard():
    ...
    try:
        ...
        total_qa_sessions = QASession.query.filter_by(user_id=current_user.id).count()
        recordings_with_qa = len(set(qa.recording_id for qa in QASession.query.filter_by(user_id=current_user.id).all()))
    except ImportError:
        # QASession model not available yet (before migration)
        total_qa_sessions = 0
        recordings_with_qa = 0

    return render_template('dashboard/index.html',
                           user=current_user,
                           recordings=recent_recordings,
                           total_qa_sessions=total_qa_sessions,
                           recordings_with_qa=recordings_with_qa)
